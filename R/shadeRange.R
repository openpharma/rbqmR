...
#' 
#' If a list of lists, each element of the  list should be a list with 
...
shadeRange <- function(
                plot,
                range=list(
                  list("lower"=NA, "upper"=0.1, "alpha"=0.3, "colour"="goldenrod1"),
              ...
              )
{
  ...
  if (is.null(range)) stop("range cannot be null")
  if (!is.list(range)) stop("range must be a list")
  ...
  if (all(sapply(range, is.list))) {
    for (r in range) {
      plot <- shadeRange(plot, r, idx)
    }
  } else {
      if (length(setdiff(names(range), c("upper", "lower", "colour", "alpha"))) != 0) {
        stop(paste0("range is not named correctly: [", paste0(names(range), collapse=", ")), "]")
      }
