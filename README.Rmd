...
[![Test Coverage](https://raw.githubusercontent.com/openpharma/rbqmR/_xml_coverage_reports/data/main/badge.svg)](https://github.com/openpharma/mtdesign/blob/_xml_coverage_reports/data/main/coverage.xml)<!-- badges: end -->
...
Yes.  Two sites are outside this range.  One above and one below.  The QTL has been breached.  

Standard QTLs, including this one, can be evaluated automatically using functions in the `rbqmR` package.

```{r evaluatePointEstimateQTL, error=TRUE}
berrySummary %>% 
  evaluatePointEstimateQTL(
    posterior = fitted$tab,
    metric = p,
