...
`evaluatePointEstimateQTL`, `evaluateXXXXQTL` and `evaluateXXXXQTL` are wrappers around `evaluateCustomQTL`, which can be used to evaluate an arbitrary, user-defined QTL rule. `evaluateCustomQTL` takews the following parameters:
